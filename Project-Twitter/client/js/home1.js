...
async function fetchBlogs() {
  ...
  if (!user.error) {
    ...
    <div class="d-flex">
      ...
      <div class="col-4 text-center">
        <a href="#" class="h4 font-bolder mb-0">${count}</a>
...
async function showTweets10(username, page, limit) {
  ...
  console.log(`http://localhost:3000/tweets/${username}/${page}/${limit}`);
...
async function postTweet() {
  ...
  const token = sessionStorage.getItem("accessToken");
  const userData = parseJwt(token);
  console.log("HERE   http://localhost:3000/tweetS");
  ...
  const result = await response.json();
  document.getElementById("numOfPosts").innerHTML = result.length;
...
(function () {
  ...
  const getQuotes = async (username, page, limit) => {
